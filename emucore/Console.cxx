...
#include "m6502/src/M6502Low.hxx"
#include "M6532.hxx"
...
//ALE   const std::string& md5 = myProperties.get(Cartridge_MD5);
...
void Console::toggleColorLoss()
{
  ...
  std::string message = std::string("PAL color-loss ") +
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void Console::setPalette(const std::string& type)
{
...
{
  const std::string& phosphor = myProperties.get(Display_Phosphor);
...
void Console::initializeVideo(bool full)
{
  if(full)
  {
    std::string title = std::string("Stella ") + STELLA_VERSION +
...
{
  const std::string& palette = myOSystem->paletteFile();
  std::ifstream in(palette.c_str(), std::ios::binary);
  if(!in)
    return;
  ...
  {
    in.close();
    std::cerr << "ERROR: invalid palette file " << palette << std::endl;
    return;
  }
