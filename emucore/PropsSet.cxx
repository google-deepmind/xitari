...
PropertiesSet::PropertiesSet(OSystem* osystem)
  : myOSystem(osystem),
    ...
    mySize(0)
{
  const std::string& props = myOSystem->propertiesFile();
  load(props, true);    // do save these properties

  if(myOSystem->settings().getBool("showinfo"))
    std::cerr << "User game properties: \'" << props << "\'\n";
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void PropertiesSet::load(const std::string& filename, bool save)
{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool PropertiesSet::save(const std::string& filename) const
{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void PropertiesSet::saveNode(std::ostream& out, TreeNode *node) const
{
  if(node)
  {
