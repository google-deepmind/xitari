    ...
    const Properties& properties, const Settings& settings)
{
  ...
  if(type == "2K")
    ...
    std::cerr << "ERROR: Invalid cartridge type " << type
      << " ..." << std::endl;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool Cartridge::saveToStream(std::ofstream& out)
{
  int size = -1;
  ...
  if(image == 0 || size <= 0)
  {
    std::cerr << "save not supported" << std::endl;
    return false;
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
std::string Cartridge::myAboutString;
