...
class M6502
{
  ...
  private:
    uint64_t PSLockupTable[256];
    uint64_t *PSPointer;
};

inline uInt8 M6502::PS() const
{
  uInt8 ps = 0x20;
...
inline void M6502::PS(uInt8 ps)
{
  *PSPointer = PSLockupTable[ps];
...
inline uint64_t M6502::PSValue(uInt8 ps)
{
  uint64_t v = 0;

  v += (ps & 0x80) > 0? 1 : 0;
  v += (uint64_t)((ps & 0x40) > 0? 1 : 0) << 8;
  v += (uint64_t)1 << 16;
  v += (uint64_t)((ps & 0x08) > 0? 1 : 0) << 24;
  v += (uint64_t)((ps & 0x04) > 0? 1 : 0) << 32;
  v += (uint64_t)((ps & 0x02) > 0? 0 : 1) << 40;
  v += (uint64_t)((ps & 0x01) > 0? 1 : 0) << 48;
  v += (uint64_t)0 << 55;
